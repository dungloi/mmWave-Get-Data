...
class RadarConfigHandler:
    ...
    def parseLine(self, cfg_line, cfg_key) -> str:
        ...
        if isinstance(cfg_line, dict):
            for key, value in cfg_line.items():
                if not isinstance(
                    value, (float, int)
                ):  # cfg_key has value which is nested
    ...
    def calcParams(self, cfg_inst) -> None:
        self.params["Carrier frequency  GHz"] = self.carrier_frequency
        self.params["Ramp Slope  MHz/us"] = cfg_inst["profileCfg"]["profile_0"][
            "freqSlopeConst"
        ]
        self.params["Num ADC Samples"] = cfg_inst["profileCfg"]["profile_0"][
            "numAdcSamples"
        ]
        self.params["ADC Sampling Rate Msps"] = (
            cfg_inst["profileCfg"]["profile_0"]["digOutSampleRate"] / 1000.0
        )

        self.params["ADC Collection Time  us"] = (
            self.params["Num ADC Samples"] / self.params["ADC Sampling Rate Msps"]
        )
        self.params["Extra ramp time required (start time)  us"] = cfg_inst[
            "profileCfg"
        ]["profile_0"]["adcStartTime"]
        self.params["Chirp time (end time - start time)  us"] = (
            cfg_inst["profileCfg"]["profile_0"]["rampEndTime"]
            - self.params["Extra ramp time required (start time)  us"]
        )
        self.params["Chirp duration (end time)  us"] = cfg_inst["profileCfg"][
            "profile_0"
        ]["rampEndTime"]
        ...
        self.params["Total BW  MHz"] = (
            self.params["Chirp duration (end time)  us"]
            * self.params["Ramp Slope  MHz/us"]
        )

        self.params["Max beat freq (80% of ADC sampling rate)  MHz"] = (
            self.params["ADC Sampling Rate Msps"] * 0.8
        )
        ...
        self.params["Range resolution  m"] = (
            self.params["c"] / 10**6 / 2 / self.params["Sweep BW (useful) MHz"]
        )
        self.params["Range resolution (meter per 1D-FFT bin)  m/bin"] = self.params[
            "Range resolution  m"
        ]

        self.params["Inter-chirp duration  us"] = cfg_inst["profileCfg"]["profile_0"][
            "idleTime"
        ]
        self.params["Number of chirp intervals in frame"] = (
            cfg_inst["frameCfg"]["chirpEndIdx"]
            - cfg_inst["frameCfg"]["chirpStartIdx"]
            + 1
        ) * cfg_inst["frameCfg"]["numLoops"]
        tx_bin = bin(cfg_inst["channelCfg"]["txChannelEn"])[2:]
        rx_bin = bin(cfg_inst["channelCfg"]["rxChannelEn"])[2:]
        ...
        self.params["Max umambiguous relative velocity  m/s"] = None  # TBD
        self.params["Max extended relative velocity  m/s"] = None  # TBD

        self.params["Frame time (total)  ms"] = (
            self.params["Number of chirp intervals in frame"]
            * (
                self.params["Inter-chirp duration  us"]
                + self.params["Chirp duration (end time)  us"]
            )
        ) / 1000
        ...
        self.params["Range FFT size"] = (
            self.params["Max distance (80%)  m"]
            / 0.8
            / self.params["Range resolution  m"]
        )
        self.params["Doppler FFT size"] = self.params[
            "Number of chirp intervals in frame"
        ]
    ...
    def genCfgFile(self, file) -> None:
        ...
        for key, value in self.config.items():
            try:
                if key == "profileCfg" and (
                    float(list(value.split())[1]) != float(self.carrier_frequency)
                ):
                ...
                if (
                    ...
                    or (
                        self.sub_frame_enabled == 0
                        and (key == "advFrameCfg" or key == "subFrameCfg")
                    )
                ):
                    continue
